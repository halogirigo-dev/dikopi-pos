...
export function EmptyStateGuide({ icon = "🗂️", title, description, actionLabel, onAction, hint }: Props) {
  return (
    <div className="card" style={{ padding:20, textAlign:"center", animation:"dikopi-enter var(--duration-emphasis) var(--ease-out) both" }}>
      ...
      {actionLabel && onAction && <button className="btn accent pressable" style={{ width:"100%", marginTop:14, minHeight:44 }} onClick={onAction}>{actionLabel}</button>}
    </div>
  );
}
