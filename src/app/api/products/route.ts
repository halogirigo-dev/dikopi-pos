import { prisma } from "@/lib/prisma";
import { getServerSession } from "next-auth";
import { authOptions } from "@/lib/auth";
import { revalidatePath } from "next/cache";

export async function GET() {
  const session: any = await getServerSession(authOptions);
  if (!session) return new Response("Unauthorized", { status: 401 });
  const products = await prisma.product.findMany({ include: { category: true }, orderBy: { name: "asc" } });
  return Response.json(products);
}
export async function POST(req: Request) {
  const session: any = await getServerSession(authOptions);
  if (!session || session.user.role !== "ADMIN") return new Response("Forbidden", { status: 403 });
  const body = await req.json();
  const { name, category_id, selling_price, image_url, is_available } = body;
  if (!name || !category_id || selling_price == null) return new Response("Missing fields", { status: 400 });
  const p = await prisma.product.create({ data: {
    name, category_id, selling_price: Number(selling_price),
    image_url: image_url || null, is_available: is_available ?? true
  }});
  revalidatePath("/pos");
  revalidatePath("/products");
  return Response.json(p);
}
